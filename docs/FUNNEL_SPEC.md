...
**Vantage · Funnel Spec v2.10 · 30 Aug 2026**
...
| **Hand-resizing is a build-time tool, not a feature** | `?resize=1` adds drag handles and a read-out panel; widths persist in that browser only. A visitor never sees it. It exists so the numbers in the stylesheet can be *chosen* and read off rather than argued about in the abstract — which is how the widths above were arrived at. |
