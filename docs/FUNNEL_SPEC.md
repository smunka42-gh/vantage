...
**Vantage · Funnel Spec v2.29 · 30 Aug 2026**
...
|---|---|---|
| **v2.29** | 30 Aug 2026 | **The card's progress bar removed.** Its length was `this company's fall ÷ the biggest fall among all 470` — a share of whichever stock happened to fall furthest that day. It described no property of the company, moved for reasons outside it, and sat directly above the 3-year band, so it read as a gauge OF that band: Monster drew 100% and Rollins 55% with the identical label beneath. Tenet 2. **The card is 302px to 210px, 30% shorter** — four on screen where two fit. Everything a reader READS moved into a left column beside the number, killing an empty half and a whole row that held two links. Those links are now `Y!` and `G` lettermarks on the meta line (a terminal chevron and a magnifier said nothing about where they went), with the expand `+` beside them. **Vocabularies shortened by MEASUREMENT**: each filter group has 327px and the direction and 3-year groups needed 346 and 364, so both wrapped. Now 301 and 286. *near 3-year low* came to 332 — five pixels under, which is not a margin worth shipping. Card chips likewise, 732 into a 664px column, now 595. The wording is shared, so cards, table and filters all move together and cannot drift. The 3-year sentence also gains precision: *the last 3 years' trading days closed higher*, which is what is measured. |
| **v2.28** | 30 Aug 2026 | **The expanded panel, audited box by box: 11 to 8, and every verdict coloured.** Pass green, fail red, could-not-be-read grey — before this every verdict rendered in the same muted grey, so a pass and a fail were indistinguishable in the one column a reader scans. Closeness is a WORD (*pass, just*) not a third colour, because near-fail IS a fail and amber would invent a middle state the grading has not got. **Stage 3's two sub-gates rendered an EMPTY verdict box** on every company — the two rows that answer that section's own question — and now read cheaper/pricier/as usual and holding/falling. Verified: 0 blanks across 470. **Two staleness cards became datelines in their heads.** They stay section-specific, and Stage 3's now carries its OWN dates — *earnings to 31 December 2025 · profit to 30 June 2026 — two different ages* — instead of repeating Stage 1's fiscal year, which is the difference the note existed to flag. **Stage 2's two boxes merged**: both stated today's price, and the scale's `(2.9%, 53.1%)` meant nothing until a footnote decoded it, so it now reads *2.9% above its 52-week low, 53.1% below its 52-week high* and the footnote is gone. **Questions rephrased** for consistency and to say what each stage measures. **The gate-table caption dropped entirely** — the head one line up is more specific in every tier (*"falls short on profit record"* beats *"Why it did not qualify"*), and it only ever carried the framing because the head had no summary; the `H` map and its stylesheet rule went with it. |
| **v2.27** | 30 Aug 2026 | **Table widths cut to what the data needs, measured across all 470.** Total 1160px to 988px. COMPANY 270 to 132 and now WRAPS rather than truncating — an ellipsis hides which company a row is about, the one thing a row must not hide; 132 of 470 names take a second line, none in the default view. Two columns could not take the full cut asked for, and the floor was measured rather than argued: `how far below usual` stops at 100px because the widest value is **-131.8%**, not 39.9% — a company priced ABOVE its usual, seven characters not five — and `size` stops at 78px on `$106.7B`. Verified by scanning every cell in all 470 rows for overflow: zero clipped. **One vocabulary for the price trend.** The filter chips read the stored values (*fell, now flat*) while the card and table for the same company read the rewording (*fell, now steady*) — two names for one state, so a reader could not connect the filter they clicked to the chip they saw. The chips now take their TEXT from the same `TREND` map the views use, keeping `data-v` as the stored value that `passes()` matches on, so the two cannot drift again. **Tenet 7 added**: the repo carries nothing about the machine it was built on. |
...
| **v2.11** | 30 Aug 2026 | **The `?resize=1` drag tool removed.** It existed to let the column widths be chosen against real data; once they were frozen in v2.10 it was ~115 lines of CSS and JS shipped to every visitor for no remaining purpose, so it is deleted rather than left gated behind a flag. Recoverable from `6abac48`. The `localStorage` override went with it, so the stylesheet is now the only thing that can set a column width. |
